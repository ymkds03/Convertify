...
@Component({
  selector: 'app-calculo-de-corriente',
  templateUrl: './calculo-de-corriente.page.html',
  styleUrls: ['./calculo-de-corriente.page.scss'],
})
export class CalculoDeCorrientePage implements OnInit {
  ...
  //traducciones
  calculos_electricos_espanol ="Cálculo de Corriente";
  calculos_electricos_ingles ="current calculation";
  ...
  factor_potencia_espanol ="Factor potencia:";
...
//calculo de potneica de motor en continua todo correcto
  calculatePotencia() {
    if (this.selectedCorriente === 'continua') {
      ...
      if (this.corriente && this.tension && this.eficiencia && this.factorPotencia) {
        this.potencia = this.corriente *  this.tension * this.factorPotencia * (this.eficiencia/100);
        this.potencia = +this.potencia.toFixed(3);
        this.potenciass = +this.potencia.toFixed(3) /1000;
        this.potenciahp = (+this.potencia.toFixed(3) /1000 )* 1.341;
      ...
      if (this.corriente && this.tension && this.eficiencia && this.factorPotencia) {
        this.potencia = Math.sqrt(3) * this.corriente * this.tension * this.factorPotencia * this.eficiencia / 100;
        this.potencia = +this.potencia.toFixed(3);
        this.potenciass = +this.potencia.toFixed(3) /1000;
        this.potenciahp = (+this.potencia.toFixed(3) /1000 )* 1.341;
      }
